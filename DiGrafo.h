...
void DiGrafica<T>::Imprime(int Opc)
{
    ...
    switch(Opc)
    {
        ...
        {       
                ...
                //respaldar la matriz en un archivo
                fstream matrizAdy("matriz_adyacencia.txt", ios::out);//Se crea el archivo logico
                if(!matrizAdy){
                    cerr<<"EL ARCHIVO NO PUDO SER ABIERTO"<<endl;
                    exit(0);
                }
                matrizAdy<<"MATRIZ DE ADYACENCIAS O COSTOS"<<endl<<endl;
                for(Ind1 = 0; Ind1 < NumVer; Ind1++){
                    matrizAdy<<Vertices[Ind1]<< ": ";
                    for(Ind2 = 0; Ind2 < NumVer; Ind2++){
                        matrizAdy<<MatAdy[Ind1][Ind2]<<"\t\t";
                    }
                    matrizAdy<<endl;
                }
                matrizAdy.close();//Se cierra el archivo
        }//Se cierra el case 0
                break;
        ...
        case 1: 
        {
                ...
                //respaldar la matriz en un archivo
                fstream warshall("Warshall.txt", ios::out);//Se crea el archivo logico
                if(!warshall){
                    cerr<<"EL ARCHIVO NO PUDO SER ABIERTO"<<endl;
                    exit(0);
                }
                warshall<<"CERRADURA TRANSITIVA DE LA MATRIZ DE ADYACENCIA"<<endl<<endl;
                for(Ind1 = 0; Ind1 < NumVer; Ind1++){
                    warshall<<Vertices[Ind1] <<": ";
                    for(Ind2 = 0; Ind2 < NumVer; Ind2++){
                        warshall<<CerTran[Ind1][Ind2]<<"\t\t";
                    }
                    warshall<<endl;
                }
                warshall.close();//Se cierra el archivo
        }//Se cierra el case 1
                break;
        ...
        case 2: 
        {
                ...
                //respaldar la matriz en un archivo
                fstream floyd("Floyd.txt", ios::out);//Se crea el archivo logico
                if(!floyd){
                    cerr<<"EL ARCHIVO NO PUDO SER ABIERTO"<<endl;
                    exit(0);
                }
                floyd<<"MATRIZ DE DISTANCIAS MINIMAS"<<endl<<endl;
                for(Ind1 = 0; Ind1 < NumVer; Ind1++){
                    floyd<<Vertices[Ind1]<< ": ";
                    for(Ind2 = 0; Ind2 < NumVer; Ind2++){
                        floyd<<MatAdy[Ind1][Ind2]<<"\t\t";
                    }
                    floyd<<endl;
                }
                floyd.clear();//Se cierra el archivo
        }//Se cierra el case 2
                break;
        ...
        case 3: 
        {
                ...
                //respaldar la matriz en un archivo
                fstream floydVerInt("FloydVerInt.txt", ios::out);//Se crea el archivo logico
                if(!floydVerInt){
                    cerr<<"EL ARCHIVO NO PUDO SER ABIERTO"<<endl;
                    exit(0);
                }
                floydVerInt<<"VERTICES INTERMEDIOS PARA LOGRAR DISTANCIAS MINIMAS"<<endl<<endl;
                for(Ind1 = 0; Ind1 < NumVer; Ind1++){
                    floydVerInt<<Vertices[Ind1]<< ": ";
                    for(Ind2 = 0; Ind2 < NumVer; Ind2++){
                        floydVerInt<<VerInt[Ind1][Ind2]<<"\t\t";
                    }
                    floydVerInt<<endl;
                }
                floydVerInt.close();//Se cierra el archivo
        }//Se cierra el case 3
                break;
        ...
        case 4: 
        {
                ...
                //Respaldar la matriz en un archivo
                fstream dijkstra("Dijkstra.txt", ios::out);//Se crea el archivo logico
                if(!dijkstra){
                    cerr<<"EL ARCHIVO NO PUDO SER ABIERTO"<<endl;
                    exit(0);
                }
                dijkstra<<"DISTANCIAS MINIMAS A PARTIR DEL VERTICE: "<<Vertices[0]<<endl<<endl;
                for(Ind1 = 0; Ind1 < NumVer; Ind1++)
                    dijkstra<<" "<<DistMin[Ind1]<<"\t"<<endl;

                dijkstra.close();//Se cierra el archivo
        }//Se cierra el case 4
                break;
